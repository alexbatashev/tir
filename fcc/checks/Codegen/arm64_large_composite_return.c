...
// callee copies into its leading pointer parameter and the state it hands back
// joins that copy's chain; the forwarding caller passes a temporary, then
// copies it into its own result address after the call.
...
// CHECK-NEXT: %[[MAKE_OUT:[0-9]+]] = state.join state(%{{[0-9]+}}, %{{[0-9]+}}, %{{[0-9]+}}, %[[MAKE_COPY]])
// CHECK-NEXT: -> %[[MAKE_OUT]]
...
// CHECK-NEXT: %[[CALL:[0-9]+]], %[[CALL_FP:[0-9]+]] = func.call %{{[0-9]+}}(%[[TEMP]]
// CHECK-SAME: ) result_address
