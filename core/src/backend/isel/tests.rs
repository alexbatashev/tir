...
#[test]
fn zero_comparison_guard_selects_zero_compare_branch() {
    let rule = Rule::new(
        "cbnz-marker",
        zero_branch_pattern(),
        LATENCY_COST_SCALE,
    ...
    let b = guarded_block_with_rules(
        &[64],
        vec![rule, materializer_rule(emit_materializer_marker)],
        |context, builder, args| {
...
fn guard_edge_arguments_are_split() {
    ...
    let before = region.iter(context.clone()).count();
    pm.run(&context, context.get_op(module.id())).unwrap();
    // The true edge's argument is forwarded through exactly one trampoline block.
    let after = region.iter(context.clone()).count();
